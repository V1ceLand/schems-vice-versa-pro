/*
 * Порт ScheduleEngine.kt на JavaScript: чётность недели, раскладка занятий
 * на дату и фаза учебного дня. Формулы и формулировки повторяют приложение,
 * поэтому демонстрация показывает не картинку, а тот же расчёт.
 */
...
/** 1 — понедельник, 7 — воскресенье, как java.time.DayOfWeek. */
...
/**
 * Занятия на дату. Замена на день полностью вытесняет постоянное расписание;
 * пометку «только сегодня» получают ровно те пары, что отличаются от недели.
 * Занятие без звонка на свой номер пары в приложении не рисуется — здесь
 * такого нет в данных, но проверка сохранена.
 */
 ...
 * прогресс до первой пары (настройка «Показывать с», по умолчанию 07:00).
 */
